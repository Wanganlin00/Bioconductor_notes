# 数据结构`SingleCellExperiment`

## 概述

`SingleCellExperiment`结构如下图 @fig-SingleCell 所示。该类实现了一个数据结构，用于存储单细胞数据的所有方面------逐细胞基因表达数据、每细胞元数据和每基因注释------并以同步的方式对其进行操作。
...
该函数允许我们获取或设置每个细胞标签的向量或因子，通常对应于由无监督聚类分析的分组（ @sec-basics5）。 或从分类算法预测细胞类型身份 （ @sex-basics7）
